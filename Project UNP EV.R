data <- read.csv("C:/Users/Ruthvik/Desktop/EV/Response1.csv",header = T)
head(data)
...
data %>% filter(Gender=="Female") %>% ggplot(aes(Govt,fill = Govt)) + geom_bar() +
theme(axis.text.x = element_text(hjust = 1,angle=90))

data %>% filter(Gender=="Male") %>% ggplot(aes(Govt,fill = Govt)) + geom_bar() +
  theme(axis.text.x = element_text(hjust = 1,angle=90))

data %>% filter(Gender=="Male") %>% ggplot(aes(OwningEV,fill = OwningEV)) + geom_bar() +
  theme(axis.text.x = element_text(hjust = 1,angle=90))

data %>% filter(Gender=="Female") %>% ggplot(aes(OwningEV,fill = OwningEV)) + geom_bar() +
  theme(axis.text.x = element_text(hjust = 1,angle=90))

data %>% ggplot(aes(AnnualIncome,fill = AnnualIncome)) + geom_bar() +
  theme(axis.text.x = element_text(hjust = 1,angle=90))
...
data %>% ggplot(aes(yes_CT,fill = yes_CT)) + 
  geom_bar()

data  %>% ggplot(aes(MoreRange,fill=MoreRange)) + geom_bar(position = "stack") + facet_grid(MoreSpeed~.) +
  theme(axis.text.x=element_text(hjust=1,angle=90))

data %>% ggplot(aes(MoreSpeed,fill = MoreSpeed)) + geom_bar() +
  theme(axis.text.x=element_text(hjust=1,angle=90))

data %>% ggplot(aes(MoreRange,fill = MoreRange)) + geom_bar() +
  theme(axis.text.x=element_text(hjust=1,angle=90))
data %>% ggplot(aes(ChareTime,fill = ChareTime)) + geom_bar() +
  theme(axis.text.x=element_text(hjust=1,angle=90))
...
colnames(data)
data <- data %>% select(-c(Govt,Qualification,EV,AnnualIncome,MoreSpeed,
                           NotPowerful,ChargingStat,DistanceBwStat,Feel,Advantages,
                           TATANEXON,TATATIGOREV,TATAULTRAT,MGZSEV))
colnames(data)
data <- data %>% select(-c(BMWi_ERIES))
colnames(data)
train <- data[1:100,]
test <- data[101:153,]
cor(data$young,data$yes_CT)

sum(data$OwningEV == "Yes")
sum(data$OwningEV == "No")
sum(data$OwningEV == "Maybe")

data$OwningEV <- unclass(data$OwningEV)-1
